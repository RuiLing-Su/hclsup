...
public class FFmpegStreamController {
    ...
    /**
     * 智能推流接口 - 自动管理通道切换和超时关闭
     *
     * @param channel 请求的通道号
     * @return AjaxResult 推流结果
     */
    @PostMapping("/smartStream")
    @Operation(summary = "combine", description = "自动管理通道切换和超时关闭的推流服务")
    public AjaxResult smartStream(@RequestParam("channel") @Parameter(description = "通道号") Integer channel) {
        ...
        if (currentChannel != null && currentChannel.equals(channel) && sessionId != null) {
            // 重置自动关闭计时器
            resetAutoCloseTimer(luserId);

            // 构建HLS路径并返回
            String hlsPath = hlsDirBasePath + "stream_" + channel + ".m3u8";
